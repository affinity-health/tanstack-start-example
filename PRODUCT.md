...
the working demo, a clinician searches a synthetic medication marketplace, adds a product to a
patient cart, reviews the cart, and confirms Test checkout.
...
synthetic patients, a medication marketplace, patient carts, and simple Test orders. Affinity Test
is the backend inside the example. WebMCP lets an agent search the catalog and fill a cart while the
clinician retains checkout and every clinical decision.
...
while checkout remains a separate clinician action.
...
- Tools may search the synthetic marketplace, open a product, add it to a named patient cart, and
  inspect the visible cart.
- Tools must not confirm checkout, create an order, prescribe, sign, submit, or transmit anything.
...
Northstar is the product on this site: a demo telehealth platform and example clinic that people can
copy. Its core workflow is marketplace search, product detail, patient cart, clinician checkout, and
simple orders. Affinity Test is the backend inside Northstar. It is never the site's offer, and the
example never connects to Live. WebMCP is optional agent wiring already included in the repository.
